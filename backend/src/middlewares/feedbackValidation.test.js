...
test('feedback validation sanitizes HTML and normalizes email', async () => {
  const { req, res, nextCalled } = await runValidation({
    name: '<b>Vivek</b>',
    email: ' VIVEK@Example.com ',
    content: 'This portfolio feels polished and informative for first-time visitors.',
  ...
  assert.equal(req.validatedFeedback.email, 'vivek@example.com');
  assert.equal(
    req.validatedFeedback.content,
    'This portfolio feels polished and informative for first-time visitors.',
  );
});

test('feedback validation rejects invalid email input', async () => {
